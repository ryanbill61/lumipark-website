...
export const PRODUCTS_QUERY = `*[_type == "product"] | order(_createdAt asc) ${PRODUCT_PROJECTION}`;

export async function fetchProducts(): Promise<Product[]> {
  try {
    const docs = await sanityView<Array<Record<string, unknown>>>("view=products");
    return (docs || [])
      .map((d) => {
        try {
          return mapProduct(d);
      ...
      .filter((p): p is Product => p !== null);  } catch {
...
export async function fetchFeaturedProducts(): Promise<Product[]> {
  try {
    const docs = await sanityView<Array<Record<string, unknown>>>("view=products&featured=true");
    return (docs || [])
      .map((d) => {
        try {
          return mapProduct(d);
