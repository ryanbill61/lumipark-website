...
export const hubSettings = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'stat_parks', title: '数字 · 园区', type: 'string', group: 'sections' }),
    defineField({ name: 'stat_factories', title: '数字 · 工厂', type: 'string', group: 'sections' }),
    defineField({ name: 'stat_years', title: '数字 · 年份', type: 'string', group: 'sections' }),
    defineField({ name: 'trust_bar_text', title: '认证底色带文字', type: 'string', group: 'sections' }),
    defineField({ name: 'mfg_h2', title: '制造实力 H2', type: 'string', group: 'sections' }),
    defineField({ name: 'mfg_lead', title: '制造实力导语', type: 'text', group: 'sections', rows: 3 }),
    defineField({ name: 'brands_h2', title: '品牌分流 H2', type: 'string', group: 'sections' }),
    defineField({ name: 'brands_copy', title: '品牌分流文案', type: 'text', group: 'sections', rows: 3 }),
    defineField({ name: 'terms_h2', title: 'B2B 条款 H2', type: 'string', group: 'sections' }),
    defineField({ name: 'terms_copy', title: 'B2B 条款文案', type: 'text', group: 'sections', rows: 3 }),
    ...footerFields,
    ...seoFields,
  ],
})
