...
export const PRODUCTS_QUERY = `*[_type == "product" && brand->name == "BMC"] | order(_createdAt asc) ${PRODUCT_PROJECTION}`;

export async function fetchProducts(): Promise<Product[]> {
  try {
    const docs = await sanityView<Array<Record<string, unknown>>>("view=products&brand=BMC");
    return (docs || [])
      .map((d) => {
        try {
          return mapProduct(d);
